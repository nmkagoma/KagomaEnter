 ...
 * Retrieves trending/popular content
 */
...
try {
    ...
    // Get trending content - weighted algorithm
    $sql = "SELECT 
    ...
    FROM content
    " . $where . "
    ORDER BY (view_count + COALESCE(rating, 0) * 100 + is_featured * 500) DESC, COALESCE(published_at, created_at) DESC
    ...
    Response::success('Trending content retrieved successfully', [
        'trending' => $trending,
        'pagination' => [
            'current_page' => $page,
            'per_page' => $limit,
    ...
    Response::serverError('An error occurred while retrieving trending content');
}
